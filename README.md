...
- **`事後` の判定はhunkの一部にしか適用できません。** 「この変更を作った編集(author)」が
  特定できたhunクだけが対象です。実測(本リポジトリ294 hunk)で **author被覆は53.7%**
  (md 65.0% / ts 52.2%)。残りは `支持(前後関係は未観測: author不明)` になります。
...
- **2026-08-04の帰属ロジック変更(連言による author 判定)は、合成ケースで機構を確認しただけで
  実データでの改善率を測っていません。** `rebuild` は保存済みイベントを再生するだけで
  derived な帰属値を再計算しないため、既存データからは測れません

### 検証済みのこと

- 自動テスト238件(vitest)が通ります。イベントストアからのprojection完全再構築、事実とclaimの峻別、プロンプトインジェクションの隔離、hookが開発をブロックしないこと、来歴チェーンの改ざん検知などを含みます。
