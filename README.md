...
**Claude Code は「セッション開始時のcwd」の設定しか読みません。** 既定の登録先はリポジトリの
`.claude/settings.json` なので、ホームディレクトリなど別の場所で起動したセッションからそのリポジトリを
編集すると、**hookは一度も発火せず、エラーも出ません**。捕捉0件と変更なしは見分けがつきません。
そういう使い方をするなら `--global` でユーザー全体設定(`$CLAUDE_CONFIG_DIR` または
`~/.claude/settings.json`)に登録してください。captureは編集対象ファイルのパスから
ワークスペースを解決するので、グローバル登録でも帰属は正しく、`.proven/` の無い場所では何もしません。
...
- **設定を決めないまま進めます。** 既定値で動いてしまうので、未決の影響は失敗ではなく判定精度の低下として現れます。
  `init` と `precheck` が未決を決め方つきで読み上げますが、**止めはしません**(決めるまで作業を塞ぐと
  「とりあえず埋める」を誘発して決定の質が下がるため)。判断は人間に残しています。

### 検証済みのこと

- 自動テスト209件(vitest)が通ります。イベントストアからのprojection完全再構築、事実とclaimの峻別、プロンプトインジェクションの隔離、hookが開発をブロックしないこと、来歴チェーンの改ざん検知などを含みます。
...
- **別リポジトリでの初導入(2026-08-02)** でも、AIが自分で足した行をunsolicited候補として最上位に出し、
  それを読み直したことで潜在バグ(rich_textの上書き)が見つかりました。同時に上記4件の設定の穴が判明しています
  (`docs/spec-setup-gaps.md`)。REQ-820〜823の修正後、仕様書を追加しただけで
  **同じ変更集合のunsolicited候補が11件→0件**になり、`spec=判定不能` が偽陽性を押し上げる経路が実データで確認できました。
- ただしこの11→0は「ズレていなかった証明」ではありません。**実装を終えてから仕様書を書いた**ので、
  後付けで辻褄を合わせただけとも読めます。これを受けてREQ-824(事後の仕様書を区別する)を実装したところ、
  **その実装自体が `spec=事後` として検出されました**(コードを書いてから仕様書を更新したため)。
  抜け穴を塞ぐ機能が、最初にその作者を捕まえた形です。
