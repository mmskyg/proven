...
### 精度: 現行コードでの実測値はありません(下記はいずれも変更前の数字)
...
- ⚠️ **この90.5%は2026-08-04の帰属ロジック変更(REQ-826/835)より前の数字です。**
  author判定の規則を変えたので、**現在のコードを表していません**。再計測が必要です。
- **claim根拠の妥当率(基準80%)は人間確認済みでは未計測です。**
  2026-08-03〜04にAI判定(`--judge ai` = `unverified`)で 50.0% → 72.7% → 85.0% と推移しましたが、
  ①AI判定は受入合否に使えない設計 ②**測定のたびに実装を変えているので数字が同じ対象を指していない**
  ③最後の測定後にも REQ-834/835/836 が入っている、の3点で**現時点の実力を示す数字ではありません**。
...
- **`事後` の判定は「この変更を作った編集(author)」を特定できたhunkだけが対象です。**
  実測(本リポジトリ294 hunk)で author 被覆は **約93%**。残り数%は
  `支持(前後関係は未観測: author不明)` になります。
  隠れた失敗ではなく、reasonに理由が出る形で degradation します
- **author判定はASCII識別子に依存します。** `isInformativeLine` の識別子抽出がASCII前提なので、
  **純日本語の行はどれだけ長く一意でも「情報がある行」と見なされません**。
  散文だけの日本語ドキュメントリポジトリでは author 被覆がほぼ0%に落ちます
- **2026-08-04の帰属ロジック変更(連言による author 判定)の効果は +1.7pt(91.2%→92.9%)** です。
  `rebuild` は保存済みイベントを再生するだけで derived な帰属値を再計算しない仕様(これは意図的で、
  過去のclaimを後から書き換えないため)なので、この測定はイベントストアからのオフライン再計算によります

### 検証済みのこと

- 自動テスト239件(vitest)が通ります。イベントストアからのprojection完全再構築、事実とclaimの峻別、プロンプトインジェクションの隔離、hookが開発をブロックしないこと、来歴チェーンの改ざん検知などを含みます。
...
- 2026-08-02に別リポジトリへ初導入し、AIが自律的に足した行をunsolicited候補として検出、
  ...
  (`docs/spec-setup-gaps.md` REQ-820〜836)。
