...
- **LLMプロバイダ**: 既定は公式APIキー方式(`anthropic`)です。Anthropicの消費者向け規約は、
  ...
  **サブスクリプション認証のCLIをバックエンドに使う経路は提供していません**。
  ...
  なお **CLI方式ではトークン使用量が取れないため、費用上限(`llm.budget_usd_per_run`)は効きません**。
  効くのは呼び出し回数の上限(`llm.max_calls_per_run`)だけです。
...
- 自動テスト194件(vitest)が通ります。イベントストアからのprojection完全再構築、事実とclaimの峻別、プロンプトインジェクションの隔離、hookが開発をブロックしないこと、来歴チェーンの改ざん検知などを含みます。
